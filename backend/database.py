...
def run_db_migrations():
    ...
    migration_sqls = [
        ...
        "CREATE TABLE IF NOT EXISTS platform_announcements (id SERIAL PRIMARY KEY, title VARCHAR(255) NOT NULL, message VARCHAR(3000) NOT NULL, priority VARCHAR(20) NOT NULL DEFAULT 'normal', target_role VARCHAR(50) NOT NULL DEFAULT 'all', is_active BOOLEAN NOT NULL DEFAULT TRUE, start_time TIMESTAMP WITH TIME ZONE, end_time TIMESTAMP WITH TIME ZONE, created_by INTEGER REFERENCES users(id) ON DELETE SET NULL, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP);",
        "ALTER TABLE platform_announcements ADD COLUMN IF NOT EXISTS target_role VARCHAR(50) DEFAULT 'all';",
